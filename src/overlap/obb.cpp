...
bool overlap::obb_overlap(const BoundingBox& box_a, const BBShape& shape_a, const BoundingBox& box_b, const BBShape& shape_b){
    auto inv_rot = box_a.rot_.inv();
    auto pos = inv_rot.rotate(box_b.pos_);
    auto rot = inv_rot * box_b.rot_;

    const auto& hsa = shape_a.half_size_;
    const auto& hsb = shape_b.half_size_;
